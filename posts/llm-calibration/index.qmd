---
title: "Do LLMs Know What They Don't Know? Teaching Confidence with RL"
date: 2026-05-25
author: "Aleksi Ikkala"
categories: [calibration, reinforcement-learning, mechanistic-interpretability]
description: "We train a 3B language model to express calibrated confidence using GRPO, then look inside to understand what changes. The main finding: the model already knew what it didn't know -- it just couldn't say so."
image: probe_results.png
---
...
1. **Representation failure**: the model simply doesn't know what it doesn't know -- the uncertainty information isn't in the model at all, and training on confidence labels can't create it from scratch.
2. **Expression failure**: the model *does* have the relevant internal signal, but lacks a mechanism to translate it into a confidence output.

These have different implications. If it's a representation problem, there's not much we could apart from training a bigger model with more capacity to store factual answers. If it's an expression problem, targeted training may be enough to *calibrate* the model translate uncertainty into the expressed confidence value. This post describes an experiment that tried to find out which one it is.

## The Setup

We finetuned Llama3.2-3B-Instruct using [GRPO](https://arxiv.org/abs/2402.03300) with a reward structure that directly incentivises calibrated confidence. For each question, the model had to produce a JSON array:

```json
["high", "Paris"]
```

where the first element is its confidence (`"high"` or `"low"`), and the second is its answer. We then rewarded or penalised based on whether it was right and how confident it claimed to be:
...
High confidence is only the rational choice when the model expects to be right more than 50% of the time. An unparseable output gets −1.5 -- worse than a low-confidence wrong answer -- so the model won't learn to output arbitrary outputs.

We trained on [PopQA](https://arxiv.org/abs/2212.10511), a factual QA dataset built from Wikidata with entity popularity scores that serve as a natural difficulty proxy. One important practical finding: training data difficulty acts as an implicit regulariser. Training on questions the model almost always gets right causes it to collapse to always saying "high confidence"; training on questions it almost never gets right causes it to always say "low confidence". We found the sweet spot at `min_popularity=20,000`, where training accuracy sits around 30–40% and both confidence labels are sometimes optimal. Training at `min_popularity=50,000` (easier questions) produced a finetuned model with ECE 0.49 -- nearly identical to the untrained base model.

We also tested two baselines using the base model without finetuning: **direct prompting** (same JSON format, no reasoning) and **chain-of-thought prompting** (brief reasoning before the answer).
...
The ECE improvement is modest (0.322 -> 0.290) compared to PopQA, and calibration gap decreases (0.366 -> 0.232). Furthermore, CoT achieves comparable calibration to the finetuned model (ECE 0.293 vs 0.290). This indicatest that the finetuned model's calibration behavior does not generalise beyond its training distribution.

**Accuracy as a side effect**

A somewhat surprising finding: calibration training also improved factual accuracy on PopQA by +3.5% ± 0.8%, statistically significant in all three seeds (McNemar's test, p < 0.005). The effect is concentrated in higher-popularity questions (+7.1% in Q4 vs +1.9% in Q1) and is absent on TriviaQA. This is consistent with RL training improving factual recall as a side effect -- a pattern also observed in [DeepSeek-R1](https://arxiv.org/abs/2402.03300) for reasoning tasks. On examples where the finetuned model expressed high confidence, the base model was already correct 55–58% of the time (vs 16.5% overall), confirming the model learned to select questions it was already close to answering correctly.

## What's Happening Inside?

Here's where it gets interesting. We trained linear probes on the residual stream activations at the position of the final prompt token -- the point right before the model starts generating -- for all 28 layers of both the base and finetuned models. This approach is similar to that presented in [Azaria & Mitchell, 2023](https://arxiv.org/abs/2304.13734), where they trained classifiers to predict the correctness of a statement based on hidden layer activations.
...
![Probe AUC across transformer layers for base model, finetuned model, and cross-dataset transfer (train on PopQA, test on TriviaQA).](probe_results.png)

**The base model already knows.** Probe AUC rises from 0.70 at layer 0 to a broad plateau of ~0.85–0.86 from layer 9 onwards. Before any calibration training, the model's internal representations are already strongly predictive of whether it will answer correctly. The uncertainty information is *there* -- the model just doesn't express it.
...
- The probe captures both genuine uncertainty information and question-difficulty surface features; finetuning on filtered training data (`min_popularity ≥ 20,000`) selectively reduces the surface-feature component.

Distinguishing between these would require causal intervention experiments -- activation patching to test whether specific layer activations causally influence the confidence output. We leave that for future work.

**Does the representation generalise across datasets?**

We trained probes on PopQA activations and evaluated on TriviaQA activations (with the test set downsampled to match the 15% PopQA positive rate). Cross-dataset AUC peaks (>0.6) in mid-layers (12–19), with near-chance performance in the first 7 layers. The early-layer signal doesn't transfer — it's capturing PopQA-specific question features. The mid-layer signal generalises, consistent with those layers encoding more abstract semantic content.
...
*Code is available at [github.com/aikkala/llm-calibration](https://github.com/aikkala/llm-calibration). Questions and feedback welcome.*
